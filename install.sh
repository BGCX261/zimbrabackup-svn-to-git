#!/bin/bash
# install.sh
# This script installs zimbrabackup on a ZCS server. It also makes sure
...
# enforced due to the way the current zimbrabackup script works.
#
...
clear
echo "This will install zimbrabackup, a script aimed at creating backups for ZCS Community Edition."
...
if [ $(id -u) -ne 0 ]; then
	echo "You need root privileges to install zimbrabackup"
	exit $ERR_NOROOT
fi
...
STATUS=0
MYDIR=`dirname $0`
test -f $MYDIR/src/zimbrabackup      || STATUS=$ERR_MISSINGFILES
test -f $MYDIR/src/zimbrarestore      || STATUS=$ERR_MISSINGFILES
test -f $MYDIR/etc/zimbrabackup.conf || STATUS=$ERR_MISSINGFILES
if ! [ $STATUS = 0 ]; then
	printf '[ERROR]\n'
	echo "Some files are missing. Please re-download the ZimbraBackup installer."
	exit $STATUS
else
	printf '[OK]\n'
fi
...
# Copy files
install -o $ZIMBRA_USER -m 700 $MYDIR/src/zimbrabackup $OSE_SRC
install -o $ZIMBRA_USER -m 700 $MYDIR/src/zimbrarestore $OSE_SRC
install --backup=numbered -o $ZIMBRA_USER -m 600 $MYDIR/etc/zimbrabackup.conf $OSE_CONF

# Add custom settings
sed -i "s|{ZIMBRA_BKPDIR}|${ZIMBRA_BKPDIR}|g" $OSE_CONF/zimbrabackup.conf
sed -i "s|{ZIMBRA_ADDRESS}|${ZIMBRA_ADDRESS}|g" $OSE_CONF/zimbrabackup.conf
sed -i "s|{ZIMBRA_ADMINPASS}|${ZIMBRA_ADMPASS}|g" $OSE_CONF/zimbrabackup.conf
sed -i "s|{ZIMBRA_LDAPPASS}|${ZIMBRA_LDAPPASS}|g" $OSE_CONF/zimbrabackup.conf
